    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden p-10 shadow-sm sm:rounded-lg flex flex-col gap-y-5">
                ...
                @forelse($my_orders as $order)                
                    <div class="item-product flex flex-row justify-between items-center">
                        <div class="flex flex-row items-center gap-x-5">
                            ...
                            <div>
                                <h3 class="text-indigo-950 font-bold text-l">{{$order->product->name}}</h3>
                                <p class="text-slate-500 font-bold text-sm">{{$order->product->category->name}}</p>
                            </div>
                        </div>
                        <div>
                ...
                @empty
                    <p>Belum ada pembelian produk</p>
                @endforelse
            </div>
        </div>
    </div>
