                ...
                @forelse($my_transactions as $transaction)                
                    <div class="item-product flex flex-row justify-between items-center">
                        <div class="flex flex-row items-center gap-x-5">
                            ...
                            <div>
                                <h3 class="text-indigo-950 font-bold text-l">{{$transaction->product->name}}</h3>
                                <p class="text-slate-500 font-bold text-sm">{{$transaction->product->category->name}}</p>
                            </div>
                        </div>
                        <div>
                            <p class="text-slate-500 font-bold text-sm">Total Price</p>
                            <p>Rp. {{number_format($transaction->product->price)}}</p>
                        </div>
                        <div>
                            <p class="text-slate-500 font-bold text-sm">Status</p>
